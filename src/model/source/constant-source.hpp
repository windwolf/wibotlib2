...
template <typename T, u8 CHANNELS>
ConstantSource<T, CHANNELS>::ConstantSource(T defaultValue) {
    // 初始化所有通道为默认值
...
template <typename T, u8 CHANNELS>
void ConstantSource<T, CHANNELS>::update() {
    // 常量源不需要更新操作，值保持不变
    // 这个方法为了实现接口而保留，实际不执行任何操作
}

template <typename T, u8 CHANNELS>
void ConstantSource<T, CHANNELS>::reset() {
    // 重置所有通道为默认构造的值
    for (u8 ch = 0; ch < CHANNELS; ch++) {
        _values[ch] = T{};
    }
...
template <typename T, u8 CHANNELS>
T ConstantSource<T, CHANNELS>::getValue(u8 channel) const {
    if (channel >= CHANNELS) {
        return T{};  // 返回默认构造的值
    }
...
template <typename T, u8 CHANNELS>
T* ConstantSource<T, CHANNELS>::getValues() const {
    return const_cast<T*>(_values);
}

template <typename T, u8 CHANNELS>
void ConstantSource<T, CHANNELS>::setValue(u8 channel, T value) {
    if (channel < CHANNELS) {
...
template <typename T, u8 CHANNELS>
void ConstantSource<T, CHANNELS>::setAllValues(T value) {
    for (u8 ch = 0; ch < CHANNELS; ch++) {
        _values[ch] = value;
    }
}
...
template <typename T, u8 CHANNELS>
void ConstantSource<T, CHANNELS>::setValues(const T values[CHANNELS]) {
    for (u8 ch = 0; ch < CHANNELS; ch++) {
...
template <typename T, u8 CHANNELS>
T ConstantSource<T, CHANNELS>::getConstantValue(u8 channel) const {
    if (channel >= CHANNELS) {
        return T{};  // 返回默认构造的值
    }
    return _values[channel];
