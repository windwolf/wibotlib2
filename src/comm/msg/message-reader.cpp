...
MessageReader::MessageReader(AsyncEventSource* source, CircularBuffer8& buffer,
                             const MessageSchema& schema, bool interFrameGap, FeedEvent feedEvents)
...
Result MessageReader::open() {
    _buffer.clear();
    _mp.reset();
    _rxWaitHandler = _source.subscribe(_feedEvents);
    return _source.start(_buffer);
}
...
Result MessageReader::read(MessageFrame& frame, u32 timeout) {
    ...
    do {
        ...
        if (rst.isError()) {
            ...
            _rxWaitHandler = _source.subscribe(_feedEvents);
            auto startRst  = _source.start(_buffer);
            if (!startRst.isOk()) {
    ...
    return _mp.parse(&frame, _interFrameGap);
}
