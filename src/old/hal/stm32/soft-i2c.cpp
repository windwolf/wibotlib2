...
SoftI2cMaster::SoftI2cMaster(GPIO_TypeDef *sclPort, u32 sclPin, GPIO_TypeDef *sdaPort, u32 sdaPin)
    ...
    I2cMasterBaseConfig cfg = {.frequency = 100000, .timeout = 1000, .stretch = false};
    setBaseConfig(cfg);
...
Result SoftI2cMaster::setBaseConfig(I2cMasterBaseConfig &config) {
...
Result SoftI2cMaster::readReg(u16 regAddr, void *data, u32 size, WaitHandler &waitHandler) {
    ...
    do {
        ...
        auto pData = static_cast<u8 *>(data);
        auto len   = size * (toUnderlying(_transitionConfig.dataWidth));
        for (u32 i = 0; i < len; i++) {
...
Result SoftI2cMaster::writeReg(u16 regAddr, void *data, u32 size, WaitHandler &waitHandler) {
    Result rst = Result::kOk;
    do {
        rst = _i2cStart();
        if (rst != Result::kOk) break;
...
Result SoftI2cMaster::read(void *data, u32 size, WaitHandler &waitHandler) {
    Result rst = Result::kOk;
    do {
        rst = _i2cStart();
        if (rst != Result::kOk) break;
...
Result SoftI2cMaster::write(void *data, u32 size, WaitHandler &waitHandler) {
    Result rst = Result::kOk;
    do {
        rst = _i2cStart();
        if (rst != Result::kOk) break;
